...
from .layers import Mlp, PatchEmbed, PatchEmbed3d, SwiGLUFFNFused, MemEffAttention, NestedTensorBlock as Block
...
class DinoVisionTransformer3d(DinoVisionTransformer):
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            in_chans (int): number of input channels
            embed_layer (nn.Module): patch embedding layer
            **kwargs: arguments passed to the DinoVisionTransformer constructor
        """
        super().__init__(
            img_size=img_size,
    ...
    def interpolate_pos_encoding(self, x, w, h, d):
        # interpolate 3d position embedding
        previous_dtype = x.dtype
        npatch = x.shape[1] - 1
        N = self.pos_embed.shape[1] - 1
        if npatch == N and w == h and w == d:
            return self.pos_embed
        pos_embed = self.pos_embed.float()
        class_pos_embed = pos_embed[:, 0]
        patch_pos_embed = pos_embed[:, 1:]
        ...
        cbrt_N = math.pow(N, (1/3))
        sx, sy, sz = float(w0) / cbrt_N, float(h0) / cbrt_N, float(d0) / cbrt_N
        patch_pos_embed = nn.functional.interpolate(
            patch_pos_embed.reshape(1, round(cbrt_N), round(cbrt_N), round(cbrt_N), dim).permute(0, 4, 1, 2, 3),
    ...
    def get_self_attention(self, x, reshape=False):
        B, _, w, h, d = x.shape
...
def vit_base_3d(patch_size=16, **kwargs):
